...
const Chats = () => {
        ...
        const unsubscribe = onSnapshot(q, async (snapshot) => {
            ...
            await Promise.all(roomsData.map(async (room) => {
                ...
                if (!participantsInfo[otherUserId]) {
                    try {
                        const { data } = await axios.get(`${baseURL}/users/get-user/${otherUserId}`);
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <View style={{ padding: 5, backgroundColor: 'white', marginTop: 5, borderRadius: 20 }}>
                <TouchableOpacity
                ...
                >
                    <Image
                        source={{ uri: receiver?.image[0]?.url || 'https://via.placeholder.com/56' }}
                        style={styles.avatar}
                    />
                    ...
                    <FontAwesome name="comments" size={24} color="black" style={{ marginRight: 20 }} />
                </TouchableOpacity >
            </View>
        );
    };
